...
channel = "##bottestingchannel"  # Channel
botnick = "testingbot"  # Your bots nick
my_master = "pyr0"
lines = 0
# regexes = [".*"]
...
def whisper(msg, user):  # whisper to a user
...
def sed(msg):
    ...
    # if there is no text between the first and last '/' (as in 's//something') do nothing because there is nothing to
    # find.
    if sedtest == '':
    ...
    # loop through the chat log and search through the messages for a match to the 'find' string. Ignore any errors
    # -- to be fixed.
    for i in content:
        try:
    ...
    if replaced == ".__not found__.":
        ...
        # if the default replaced text was found, perform a replace on the text, strip any newlines, and send to
        # checksend method for verification before sending to channel.
...
def regex(msg):
    ...
    # get the text between the first and last '|' as the 'find portion
    sedtest = msg.split('|', 1)[1].rsplit('|', 1)[0]
    # if there is no text between the first and last '|' (as in 's||somethin') do nothing because there is nothing to
    # find.
    if sedtest == '':
        print("Nothing to find")
        return
    ...
    for i in content:
        try:
            ...
            text = i.split(":", 1)[1]
            # search the text for regex pattern, if found, set found equal to the text and replace it with replace
            # string. Set name equal to the username of found message.
    ...
    if repltext[1] > 10:
        ...
        return
    # set replaced string equal to just the new message and strip newlines, then send to checksend for verification
...
# log chat messages
def logger(name, msg):
    # loop through the content of the chat log and reduce to 100 lines, starting with oldest. --Definitely a better
    # way to do this, needs improvement.
    with open("ircchat.log", 'r') as irclog:
        content = irclog.readlines()

    # loop through the content of the chat log and reduce to 100 lines, starting with oldest. --Definitely a better
    # way to do this, needs improvement.
    with open("ircchat.log", "w") as irclog:
...
def checksend(name, orig, new, pattern = ''):
    ...
    if pattern in {'\\s', '\\S', '\\D', '\\W', '\\w'}:
        whisper(
            "Wildcard(s) not allowed because they are too broad. If you meant to seach plaintext use 's/[find]/["
            "replace]' or delimit the wildcard (like s|\\\\s|!s",
            name)
        return
    ...
    if len(new) == 0:
        whisper("Replace resulted in empty message. Nothing to send", name)
        return
...
def help(name, topic = ''):
    ...
    if topic == '':
        message = "Hi! I am an irc bot created by deepgorthi. Current function is find and replace. You can perform " \
                  "a plain text find/replace on recent chat by typing in \'s/[find]/[replace]\' (no quotes) or a " \
                  "regex based find/replace with \'s|[find]|[replace]\'. Note that some wildcards are disabled " \
                  "because they result in too many matches."
    else:
...
def main():
    ...
    joinchan(channel)
    # Show time joined
    print(f"Hi there, {botnick} joined at ", end = "")
    display_time()
    bot_joined = f'Hi {my_master}, I humbly introduce myself to you as {botnick} and I am at your service from ' + \
                 str(time.ctime()) + f" and until you command me to leave with 'gtfo {botnick}'"
    sendmsg(bot_joined)
    ...
    while 1:
        ...
        print(ircmsg)
        # respond to pings so server doesn't think we've disconnected
        ...
        if ircmsg.find("PRIVMSG") != -1:
            ...
            if message[:2] == 's|':
                ...
                if len(name) < 17:
                    ...
                    if name.lower() == my_master and message[:5 + len(botnick)] == "gtfo %s" % botnick:
                        sendmsg("Oh...ok. Will do as you command :'(")
                        print(f'{botnick} is leaving at ', end = "")
                        display_time()
                        time_left = f'{botnick} is leaving at ' + str(time.ctime())
                        sendmsg(time_left)
                        ircsock.send(bytes("PART " + channel + "\r\n", "UTF-8"))
                        sys.exit()


# start main function to run the module as a script
